...
def find_last_processed_model(temp_dir):
    last_model_id = None
...
def main():
    ...
    list_limits = {
        'openlab08.umiacs.umd.edu': (444, 500),
        'openlab30.umiacs.umd.edu': (227, 299),
        'openlab31.umiacs.umd.edu': (328, 400),
        'openlab32.umiacs.umd.edu': (731, 799),
        'openlab33.umiacs.umd.edu': (825, 900),
    }

    cpu_count = 5 if socket.gethostname() == 'openlab08.umiacs.umd.edu' else 10
    ...
    experiment_name = f'squares-all-classes-gray_CPU_{lim_left}-{lim_right}'
    ...
    path_temp_dir = os.path.join(path_root, 'ics_svm', f'TEMP_{experiment_name}')
    af.create_path(path_temp_dir)

    metadata = pd.read_csv(path_metadata)
    mp_mapping_params = []

    last_model_name_in_temp_dir = find_last_processed_model(path_temp_dir)

    for _, row in metadata.iterrows():
        model_name = row['model_name']
        model_id = int(model_name[3:])
        if (lim_left <= model_id <= lim_right) and ((last_model_name_in_temp_dir is None) or (last_model_name_in_temp_dir is not None and model_name > last_model_name_in_temp_dir)):
    ...
    with Pool(max_workers=cpu_count) as pool:
        ...
        df.sort_values(by=['model_name'], ascending=True, inplace=True)
