...
class SDNInception3(SDNTrojAI):
    def __init__(self, model, input_size, num_classes, sdn_type, device):
        super(SDNInception3, self).__init__(model, input_size, num_classes, sdn_type, device)
    ...
    def forward_w_acts(self, x):
        ...
        # N x 192 x 35 x 35
        x = self.model.Mixed_5b(x)  # Inception A
        # N x 256 x 35 x 35
        x = self.model.Mixed_5c(x)  # Inception A
        # N x 288 x 35 x 35
        x = self.model.Mixed_5d(x)  # Inception A
        # N x 288 x 35 x 35
        x = self.model.Mixed_6a(x)  # Inception B

        # add SDN output (IC)
        activations.append(af.reduce_activation(x))

        # N x 768 x 17 x 17
        x = self.model.Mixed_6b(x)  # Inception C
        # N x 768 x 17 x 17
        x = self.model.Mixed_6c(x)  # Inception C
        # N x 768 x 17 x 17
        x = self.model.Mixed_6d(x)  # Inception C
        # N x 768 x 17 x 17
        x = self.model.Mixed_6e(x)  # Inception C
        # N x 768 x 17 x 17
        aux_defined = self.model.training and self.model.aux_logits
        if aux_defined:
            aux = self.model.AuxLogits(x)
        else:
            aux = None
        # N x 768 x 17 x 17
        x = self.model.Mixed_7a(x)  # Inception D

        # add SDN output (IC)
        activations.append(af.reduce_activation(x))

        # N x 1280 x 8 x 8
        x = self.model.Mixed_7b(x)  # Inception E
        # N x 2048 x 8 x 8
        x = self.model.Mixed_7c(x)  # Inception E
        # N x 2048 x 8 x 8
