...
def _forward_w_acts_for_end_DenseBlock(x, net_features):
    activations = []
    out = None
...
def _forward_w_acts_for_inside_DenseBlock(x, net_features):
    activations = []
    out = x
...
def _get_layerwise_params_end_DenseBlock(x, net_features, num_classes):
...
def _get_layerwise_params_inside_DenseBlock(x, net_features, num_classes):
    params = [] # will contain tuples (self.num_classes, current_input_size, input_features_for_current_IC)
    # Returns the logits of each DenseLayer inside any DenseBlock. Will attach to many ICs
    for layer in net_features:
        # print(type(layer))
...
class SDNDenseNet121(nn.Module):
    ...
    def forward_w_acts(self, x):
        ...
        if self.sdn_type == SDNConfig.DenseNet_end_DenseBlock:
            activations, out = _forward_w_acts_for_end_DenseBlock(x, net_features)
        elif self.sdn_type == SDNConfig.DenseNet_inside_DenseBlock:
            activations, out = _forward_w_acts_for_inside_DenseBlock(x, net_features)
        else:
    ...
    def get_layerwise_model_params(self):
        ...
        if self.sdn_type == SDNConfig.DenseNet_end_DenseBlock:  # attach_IC_at_DenseBlock_end
            return _get_layerwise_params_end_DenseBlock(x, net_features, self.num_classes)
        elif self.sdn_type == SDNConfig.DenseNet_inside_DenseBlock:  # attach_IC_at_each_DenseLayer_in_DenseBlock
            return _get_layerwise_params_inside_DenseBlock(x, net_features, self.num_classes)
        else:
